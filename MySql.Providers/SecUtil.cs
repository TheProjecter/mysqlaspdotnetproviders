...
namespace MySql.Providers {
    ...
    internal static class SecUtility {
        ...
        internal static void CheckSchemaVersion(ProviderBase provider, MySqlConnection connection, string[] features, string version, ref int schemaVersionCheck) {
            if (connection == null) {
...
//                        int iStatus = 0;
                        ...
                        foreach (string feature in features) {
                            String sQuery = "SELECT COUNT(*) FROM aspnet_schemaversions WHERE feature = @feature AND compatibleschemaversion = @compatibleschemaversion";

                            cmd = new MySqlCommand(sQuery, connection);

//                            cmd = new SqlCommand("dbo.aspnet_CheckSchemaVersion", connection);

//                            cmd.CommandType = CommandType.StoredProcedure;

                            p = new MySqlParameter("@Feature", feature);
                            cmd.Parameters.Add(p);

                            p = new MySqlParameter("@CompatibleSchemaVersion", version);
                            cmd.Parameters.Add(p);
